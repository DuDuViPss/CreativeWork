...
void MainWindow::NewSearch()
{
    ...
    /* формирование итогового списка возможных путей */
        ...
        ui->label->setText("Результат: " + qstr);
        /* очищение памяти */
        for (int i = 0; i < r;i++)
            delete[] matr[i];
        for (int i = 0; i < r;i++)
...
{
    ui->gotab->setStyleSheet("color: rgb(250, 0, 0); background-color: rgb(51,51,204); border: 1px solid rgb(0, 0, 0); border-radius: 10px;");
}
void MainWindow::on_build_pressed()
{
    ui->build->setStyleSheet("color: rgb(250, 0, 0); background-color: rgb(51,51,204); border: 1px solid rgb(0, 0, 0); border-radius: 10px;");
}
void MainWindow::on_go_pressed()
{
    ui->go->setStyleSheet("color: rgb(250, 0, 0); background-color: rgb(51,51,204); border: 1px solid rgb(0, 0, 0); border-radius: 10px;");
}
